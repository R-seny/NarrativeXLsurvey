...
function resizeOutputGrid() {
    ...
    var height = $('#output_grid_size_h').val();
    var width = $('#output_grid_size_w').val();
    var size = checkSize(height, width);
    height = size[0];
    width = size[1];
...
function copyFromInput() {
    ...
    $('#output_grid_size_h').val(CURRENT_OUTPUT_GRID.height);
    $('#output_grid_size_w').val(CURRENT_OUTPUT_GRID.width);
}
...
function loadJSONTask(train, test) {
    ...
    //console.log("Trying to show the task")
    //console.log(train)
...
function myLoadJSONTask(train, test, current_test_subproblem) {
    resetTask();
    $('#modal_bg').hide();
    $('#error_display').hide();
    $('#info_display').hide();
    ...
    //    TEST_PAIRS.push(pair);
    //}
    // Todo select test index and save it as a global variable
    //CURRENT_TEST_SUBPROBLEM = Math.floor(Math.random() * test.length);

    if (current_test_subproblem >= test.length) {
        console.log("In myLoadJson: CURRENT TEST SUBPROBLEM larger than the number of test suproblems!");
    }

    pair = test[current_test_subproblem];
    TEST_PAIRS.push(pair);
...
function myRandomTask() {
    ...
    $("#solution_description").hide();
}

function submitTextDescription() {
    console.log($('#text_description').val());
...
function mySubmitSolution() {
    ...
    // My edite submit solution for te interface
    syncFromEditionGridToDataGrid();
    reference_output = TEST_PAIRS[CURRENT_TEST_PAIR_INDEX]['output'];
    submitted_output = CURRENT_OUTPUT_GRID.grid;
    if (reference_output.length != submitted_output.length) {
